...
.equal-height-columns h1 {
	margin-bottom: 20px;
}
	
.equal-height-columns h5 {
	margin-bottom: 20px;
	color: #888;
}
...
<body>
	...
	<div class="equal-columns-row">
		<div class="equal-height-columns columns-aside"></div>

		<div class="equal-height-columns">
			<h1>${news.title}</h1>
			<h5>${news.author} | <fmt:parseDate value="${news.time}" var="date" pattern="yyyy-MM-dd HH:mm:ss"/><fmt:formatDate value="${date}" pattern="yyyy-MM-dd HH:mm:ss" /></h5>
			<p>${news.content}</p>
		</div>
